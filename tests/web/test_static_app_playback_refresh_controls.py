...
const activeSettings = {
  voice_stack: { mode: "live_ephemeral", loop_seconds: 75, transition_seconds: 0 },
  input_control: {
    minimum_recording_seconds: 3,
    maximum_recording_seconds: 120,
  },
  ...
  audio: { sample_rate: 48000, channels: 2, loop_seconds: 300 },
  devices: { input_device_id: "mic-1", output_device_id: "speaker-1" },
  playback: { auto_start: true, apply_mode: "live", master_volume_db: -9 },
  sources: {
    low_path: null,
...
"""
    ...
    run_node_harness(
        script=app_script,
        body="""
const { applyState } = globalThis.__secretPond;
...
def test_playback_timeline_falls_back_to_frame_cursor_when_position_is_missing() -> None:
    app_script = Path("src/secret_pond/web/static/app.js").read_text(encoding="utf-8")
    app_script = app_script.replace(STATIC_APP_BOOTSTRAP, "")
    app_script += """
globalThis.__secretPond = {
...
def test_live_seek_slider_input_scrubs_playback_immediately() -> None:
    app_script = Path("src/secret_pond/web/static/app.js").read_text(encoding="utf-8")
    app_script = app_script.replace(STATIC_APP_BOOTSTRAP, "")
    app_script += """
globalThis.__secretPond = {
...
seekSlider.value = "45";
seekSlider.dispatchEvent({ type: "input", target: seekSlider });
...
applyState({
  ...
  playback: {
    ...
    duration_seconds: 60,
    progress: 0,
    live: { enabled: false, seek_applies_immediately: false },
  },
  armed: false,
  is_recording: false,
  recording_elapsed_seconds: 0,
...
def test_prepared_playback_enables_seek_slider_before_output_starts() -> None:
    app_script = Path("src/secret_pond/web/static/app.js").read_text(encoding="utf-8")
    app_script = app_script.replace(STATIC_APP_BOOTSTRAP, "")
    app_script += """
globalThis.__secretPond = {
...
const liveSettings = {
  voice_stack: { mode: "live_ephemeral", loop_seconds: 60, transition_seconds: 0 },
  input_control: {
    minimum_recording_seconds: 3,
    maximum_recording_seconds: 120,
  },
...
def test_live_successful_save_active_settings_become_later_failure_rollback_source() -> None:
    app_script = Path("src/secret_pond/web/static/app.js").read_text(encoding="utf-8")
    app_script = app_script.replace(STATIC_APP_BOOTSTRAP, "")
    app_script += """
globalThis.__secretPond = {
...
state.snapshot = {
  ...
  settings: {
    active: cloneSettings(activeSettings),
    draft: cloneSettings(activeSettings),
    change: cloneSettings(changePayload),
  },
  armed: false,
  is_recording: false,
  recording_elapsed_seconds: 0,
...
commitDraftChange(
  () => {
    state.draft.layers.low.volume_db = -3;
  },
  { feedbackControlId: "layers.low.volume_db", scheduleSave: false },
);

globalThis.fetch = async () => {
  const confirmedSettings = cloneSettings(activeSettings);
  confirmedSettings.layers.low.volume_db = -3;
  return {
    ok: true,
    status: 200,
    json: async () => ({
      settings: {
        active: cloneSettings(confirmedSettings),
        draft: cloneSettings(confirmedSettings),
        change: cloneSettings(changePayload),
      },
    }),
  };
...
assert.strictEqual(state.snapshot.settings.active.layers.low.volume_db, -3);

state.snapshot.settings.active.layers.low.volume_db = 0;
commitDraftChange(
  () => {
    state.draft.layers.low.volume_db = -6;
  },
...
def test_live_layer_preset_failure_preserves_untouched_same_card_control() -> None:
    app_script = Path("src/secret_pond/web/static/app.js").read_text(encoding="utf-8")
    app_script = app_script.replace(STATIC_APP_BOOTSTRAP, "")
    app_script += """
globalThis.__secretPond = {
...
"""
    ...
    run_node_harness(
        ...
        body="""
(async () => {
const { applyLayerPreset, saveDraft, state } = globalThis.__secretPond;
...
def test_playback_apply_mode_click_shows_requested_mode_while_request_is_pending() -> None:
    app_script = Path("src/secret_pond/web/static/app.js").read_text(encoding="utf-8")
    app_script = app_script.replace(STATIC_APP_BOOTSTRAP, "")
    app_script += """
globalThis.__secretPond = {
...
const pendingSwitch = setPlaybackApplyMode("live");

assert.strictEqual(state.playbackApplyModeInFlight, true);
assert.match(panel.className, /\\blive\\b/);
assert.match(panel.className, /\\bpending\\b/);
assert.strictEqual(panel.getAttribute("aria-busy"), "true");
assert.strictEqual(
  document.getElementById("playbackApplyModeSummary").textContent,
...
assert.strictEqual(stableButton.getAttribute("aria-pressed"), "false");
assert.strictEqual(liveButton.classList.contains("active"), true);
assert.strictEqual(liveButton.classList.contains("pending"), true);
assert.strictEqual(stableButton.classList.contains("active"), false);
assert.strictEqual(stableButton.classList.contains("pending"), false);
assert.strictEqual(liveButton.disabled, true);
assert.strictEqual(stableButton.disabled, true);
...
def test_playback_apply_mode_live_switch_confirms_before_discarding_staged_stable_changes() -> None:
    app_script = Path("src/secret_pond/web/static/app.js").read_text(encoding="utf-8")
    app_script = app_script.replace(STATIC_APP_BOOTSTRAP, "")
    app_script += """
globalThis.__secretPond = {
...
draftSettings.layers.low.volume_db = -12;
draftSettings.playback.master_volume_db = -3;
...
applyState({
  settings: {
    ...
    change: {
      changed_sections: ["layers", "playback"],
      requires_restart: false,
      runtime_config_changed: false,
    },
  },
...
const staleStableSettings = cloneSettings(liveSettings);
staleStableSettings.playback.apply_mode = "stable";
...
const liveButton = document.getElementById("playbackApplyModeLiveButton");
const stableButton = document.getElementById("playbackApplyModeStableButton");
document.getElementById("playbackApplyModePanel").append(liveButton, stableButton);
assert.strictEqual(liveButton.getAttribute("aria-pressed"), "true");
assert.strictEqual(stableButton.getAttribute("aria-pressed"), "false");
...
applyState({
  settings: {
    ...
    draft: cloneSettings(staleStableSettings),
    change: { changed_sections: [], requires_restart: false, runtime_config_changed: false },
  },
  playback: { output_running: true, frame_cursor: 1800, apply_mode: "stable" },
  armed: false,
  is_recording: false,
  recording_elapsed_seconds: 0,
  recording_remaining_seconds: 120,
...
def test_live_channel_count_draft_shows_apply_required_message() -> None:
    app_script = Path("src/secret_pond/web/static/app.js").read_text(encoding="utf-8")
    app_script = app_script.replace(STATIC_APP_BOOTSTRAP, "")
    app_script += """
globalThis.__secretPond = {
...
def test_active_live_eq_control_group_survives_state_refresh() -> None:
    app_script = Path("src/secret_pond/web/static/app.js").read_text(encoding="utf-8")
    app_script = app_script.replace(STATIC_APP_BOOTSTRAP, "")
    app_script += """
globalThis.__secretPond = {
...
const liveEqSlider = document.createElement("input");
liveEqSlider.type = "range";
liveEqSlider.value = "3";
layerControls.appendChild(liveEqSlider);
document.activeElement = liveEqSlider;
trackInteractiveControl(liveEqSlider);

const refreshedSettings = cloneSettings(liveSettings);
refreshedSettings.layers.low.eq.low_gain_db = -6;
applyState({
  settings: {
    active: refreshedSettings,
    draft: refreshedSettings,
...
assert.strictEqual(document.getElementById("layerControls"), layerControls);
assert.strictEqual(layerControls.children.includes(liveEqSlider), true);
assert.strictEqual(liveEqSlider.value, "3");
assert.strictEqual(state.activeInteractiveControl, liveEqSlider);
assert.strictEqual(
  state.deferredInteractiveRenders["settings-controls"].name,
  "renderControls",
);
