...
class FailingResetDraftSettingsStore:
    def __init__(self, delegate, error: Exception) -> None:
        self.delegate = delegate
        self.error = error
...
def run_static_app_harness(
    ...
    *,
    ...
    run_node_harness(script, body=body, dom_setup=dom_setup)
...
def draft_with_voice_layer_disabled(*, base: AppSettings | None = None) -> dict:
...
def draft_with_low_volume(volume_db: float, *, base: AppSettings | None = None) -> dict:
    ...
    layers = {
        **settings.layers,
        "low": settings.layers["low"].model_copy(update={"volume_db": volume_db}),
    }
    return settings.model_copy(update={"layers": layers}, deep=True).model_dump(mode="json")


def draft_with_mid_volume(volume_db: float, *, base: AppSettings | None = None) -> dict:
    settings = base or api_settings()
    layers = {
        **settings.layers,
        "mid": settings.layers["mid"].model_copy(update={"volume_db": volume_db}),
    }
    return settings.model_copy(update={"layers": layers}, deep=True).model_dump(mode="json")
...
def api_settings_with_voice_only_live_playback() -> AppSettings:
    ...
    layers = {
        **settings.layers,
        "low": settings.layers["low"].model_copy(update={"enabled": False}),
        "mid": settings.layers["mid"].model_copy(update={"enabled": False}),
        "voice": settings.layers["voice"].model_copy(update={"enabled": True, "volume_db": 0.0}),
    }
    return settings.model_copy(update={"layers": layers}, deep=True)


def api_settings_with_mid_only_live_playback() -> AppSettings:
    settings = api_settings().model_copy(
        update={"playback": PlaybackSettings(apply_mode="live")},
        deep=True,
    )
    layers = {
        **settings.layers,
        "low": settings.layers["low"].model_copy(update={"enabled": False}),
        "mid": settings.layers["mid"].model_copy(update={"enabled": True, "volume_db": 0.0}),
        "voice": settings.layers["voice"].model_copy(update={"enabled": False}),
    }
    return settings.model_copy(update={"layers": layers}, deep=True)


def api_settings_with_low_only_live_playback() -> AppSettings:
    settings = api_settings().model_copy(
        update={"playback": PlaybackSettings(apply_mode="live")},
        deep=True,
    )
    layers = {
        **settings.layers,
        "low": settings.layers["low"].model_copy(update={"enabled": True, "volume_db": 0.0}),
        "mid": settings.layers["mid"].model_copy(update={"enabled": False}),
        "voice": settings.layers["voice"].model_copy(update={"enabled": False}),
    }
    return settings.model_copy(update={"layers": layers}, deep=True)
...
def draft_with_audio_loop_seconds(
    loop_seconds: int,
    *,
    ...
    settings = base or api_settings()
    return settings.model_copy(
        update={"audio": settings.audio.model_copy(update={"loop_seconds": loop_seconds})},
        deep=True,
    ).model_dump(mode="json")
...
(async () => {
  ...
  assert.strictEqual(helpers.state.snapshot.participant_count, 4);
  assert.strictEqual(helpers.state.snapshot.playback.apply_mode, "stable");
  assert.strictEqual(helpers.state.snapshot.playback.output_running, false);
  assert.strictEqual(helpers.state.snapshot.playback.live.enabled, false);
  assert.strictEqual(elements.participantCount.textContent, 4);
  ...
  assert.strictEqual(helpers.state.snapshot.participant_count, 7);
  assert.strictEqual(helpers.state.snapshot.playback.apply_mode, "stable");
  assert.strictEqual(helpers.state.snapshot.playback.output_running, false);
  assert.strictEqual(helpers.state.snapshot.playback.live.enabled, false);
...
def test_api_settings_reset_draft_returns_conflict_when_save_fails(tmp_path: Path) -> None:
    client = create_test_client(tmp_path, raise_server_exceptions=False)
    client.put("/api/settings/draft", json=draft_with_voice_volume(-9.0))
    runtime = client.app.state.runtime
    runtime.settings_store = FailingResetDraftSettingsStore(
        runtime.settings_store,
        OSError("settings save failed"),
    )
    ...
    stored = SettingsStore(ProjectPaths(tmp_path)).load()
    assert stored.active.layers["voice"].volume_db == -18.0
    assert stored.draft.layers["voice"].volume_db == -9.0
...
def test_api_state_reports_playback_timeline_from_voice_loop_seconds(
    tmp_path: Path,
) -> None:
    settings = api_settings().model_copy(
        update={
            "audio": AudioFormatSettings(sample_rate=8_000, channels=2, loop_seconds=300),
...
def test_api_live_settings_draft_keeps_playback_timeline_on_active_voice_loop(
    tmp_path: Path,
) -> None:
    settings = api_settings().model_copy(
        update={"playback": PlaybackSettings(apply_mode="live")},
    ...
    assert state_after_draft["playback"]["duration_seconds"] == pytest.approx(
        settings.voice_stack.loop_seconds,
    )
    ...
    assert state_after_apply["playback"]["duration_seconds"] == pytest.approx(
        settings.voice_stack.loop_seconds,
    )
...
def test_api_playback_seek_updates_player_cycle_to_voice_loop(
    tmp_path: Path,
) -> None:
    settings = api_settings().model_copy(
        update={
            "audio": AudioFormatSettings(sample_rate=8_000, channels=2, loop_seconds=3),
            "voice_stack": VoiceStackSettings(loop_seconds=1, transition_seconds=0),
            "playback": PlaybackSettings(apply_mode="live"),
        },
        deep=True,
    )
    ...
    output.player = client.app.state.runtime.player
    client.post("/api/settings/apply-and-restart")
    start = client.post("/api/playback/start")

    response = client.post("/api/playback/seek", json={"progress": 0.75})
    block = client.app.state.runtime.player.next_block(3_000)

    assert start.status_code == 200
    assert response.status_code == 200
    assert response.json()["state"]["playback"]["frame_cursor"] == 6_000
    assert response.json()["state"]["playback"]["duration_seconds"] == pytest.approx(1.0)
    assert block.next_frame_cursor == 1_000
    assert client.app.state.runtime.player.frame_cursor == 1_000


def test_api_live_playback_seek_preserves_running_output_and_player(
    tmp_path: Path,
) -> None:
    output = PlayerLinkedFakeOutput()
    settings = api_settings().model_copy(
...
def test_api_live_settings_draft_update_applies_low_volume_without_restart(
    tmp_path: Path,
) -> None:
    output = PlayerLinkedFakeOutput()
    ...
    response = client.put(
        "/api/settings/draft",
        json=draft_with_low_volume(-12.0, base=settings),
    )
    ...
    assert output.stop_calls == 0
    assert output.start_calls == 1
    assert response.json()["settings"]["active"]["layers"]["low"]["volume_db"] == -12.0
    assert response.json()["settings"]["draft"]["layers"]["low"]["volume_db"] == -12.0
    assert client.app.state.runtime.player.layer_states["low"].realtime_trim_db == -12.0
...
def test_api_live_settings_draft_update_applies_mid_volume_without_restart(
    ...
    output = PlayerLinkedFakeOutput()
    settings = api_settings_with_mid_only_live_playback()
    client = create_test_client(tmp_path, with_sources=True, output=output, settings=settings)
    output.player = client.app.state.runtime.player
    client.post("/api/settings/apply-and-restart")
    client.post("/api/playback/start")
    ...
    response = client.put(
        "/api/settings/draft",
        json=draft_with_mid_volume(-12.0, base=settings),
    )
    after = client.app.state.runtime.player.next_block(512)
    ...
    assert output.stop_calls == 0
    assert output.start_calls == 1
    assert response.json()["settings"]["active"]["layers"]["mid"]["volume_db"] == -12.0
    assert response.json()["settings"]["draft"]["layers"]["mid"]["volume_db"] == -12.0
    assert client.app.state.runtime.player.layer_states["mid"].realtime_trim_db == -12.0
    assert float(np.max(np.abs(after.samples))) < float(np.max(np.abs(before.samples)))


def test_api_live_settings_draft_update_applies_mid_mute_without_restart(
    tmp_path: Path,
) -> None:
    output = PlayerLinkedFakeOutput()
    settings = api_settings_with_mid_only_live_playback()
    ...
    assert output.stop_calls == 0
    assert output.start_calls == 1
    assert response.json()["settings"]["active"]["layers"]["mid"]["enabled"] is False
...
def test_api_live_settings_draft_update_applies_voice_mute_without_restart(
    ...
    voice = AudioBuffer(
        samples=np.ones(
            (settings.audio.sample_rate * settings.audio.loop_seconds, settings.audio.channels),
            dtype=np.float32,
        )
        * 0.05,
        sample_rate=settings.audio.sample_rate,
    )
    write_wav_atomic(paths.voice_stack_raw, voice)
    output.player = client.app.state.runtime.player
    client.post("/api/settings/apply-and-restart")
    client.post("/api/playback/start")
    before = client.app.state.runtime.player.next_block(512)
    ...
    assert output.stop_calls == 0
    assert output.start_calls == 1
    assert response.json()["settings"]["active"]["layers"]["voice"]["enabled"] is False
