...
Use `Maintenance` > `Reset Draft` only when you want to discard unsaved draft settings. Reset Draft does not render audio, apply settings, or change the currently active playback settings.
